...
async function rewriteTextFiles(pkgDir: string) {
  const files = await globby(REWRITE_GLOBS, {
    ...
    ignore: [
      ...
      // re-matched inside the freshly written `@vertex/prisma` value).
      'package.json',
    ],
  })
  ...
  // to `'@vertex/prisma-client/runtime/library'` — the trailing `/runtime/…`
  ...
  // and URLs (e.g. `'./prisma/schema.prisma'` → `'./@vertex/prisma/schema.prisma'`
  // or `'https://github.com/lh0x00/prisma/releases/...'` →
  // `'https://github.com/lh0x00/@vertex/prisma/releases/...'`). Both
