...
`@vertex/*` (hoặc bất kỳ scope nào bạn cấu hình), bao gồm cả **Rust engines**
(host trên GitHub Releases) và 3 **WASM packages**, sao cho người dùng chỉ cần
`npm i @vertex/prisma @vertex/prisma-client` là dùng được code + engine
...
| ----------------------- | ------------------------------------- | ------------------------------------- |
| `debug`                 | `@prisma/debug`                       | `@vertex/prisma-debug`                |
| `driver-adapter-utils`  | `@prisma/driver-adapter-utils`        | `@vertex/prisma-driver-adapter-utils` |
| `adapter-pg`            | `@prisma/adapter-pg`                  | `@vertex/prisma-adapter-pg`           |
| `get-platform`          | `@prisma/get-platform`                | `@vertex/prisma-get-platform`         |
| `engines-version-fork`  | `@prisma/engines-version-fork` (stub) | `@vertex/prisma-engines-version`      |
| `fetch-engine`          | `@prisma/fetch-engine`                | `@vertex/prisma-fetch-engine`         |
| `engines`               | `@prisma/engines`                     | `@vertex/prisma-engines`              |
| `config`                | `@prisma/config`                      | `@vertex/prisma-config`               |
| `client-runtime-utils`  | `@prisma/client-runtime-utils`        | `@vertex/prisma-client-runtime-utils` |
| `client`                | `@prisma/client`                      | `@vertex/prisma-client`               |
| `cli`                   | `prisma`                              | `@vertex/prisma`                      |
...
trong các package khác được tự động đổi sang `@vertex/prisma-engines-version`.
...
| ------------------------------------ | ----------------------------- | ------------------------------------ |
| `prisma-schema-wasm`                 | `@prisma/prisma-schema-wasm`  | `@vertex/prisma-schema-wasm`         |
| `schema-engine/schema-engine-wasm`   | `@prisma/schema-engine-wasm`  | `@vertex/prisma-schema-engine-wasm`  |
| `query-compiler/query-compiler-wasm` | `@prisma/query-compiler-wasm` | `@vertex/prisma-query-compiler-wasm` |
...
- `npm login --scope=@vertex` (hoặc `NPM_TOKEN` qua `~/.npmrc`).
...
> **Tại sao tách `patch-source` ↔ `rebrand`**: Nếu `rebrand` chạy trước build, nó sẽ xóa `devDependencies` của `@vertex/prisma-client`, làm Turborepo mất signal về dep chain `client → internals → get-dmmf` (chain qua devDeps). Hệ quả: turbo schedule song song → race condition, build fail với `Could not resolve "@prisma/get-dmmf"`. Vì vậy `rebrand` phải chạy SAU build.
...
| `FORK_SCOPE`          | `--scope`    | `@vertex`                                                    | Scope npm                                                        |
...
```bash
npm i -D @vertex/prisma
npm i    @vertex/prisma-client
```

Generator output (`prisma generate`) sẽ tự `require('@vertex/prisma-client/runtime/library')`
